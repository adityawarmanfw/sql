<script>
    import {
        ...
        sql,
        DuckDBDialect,
    } from "$lib/playground/codemirror.js";
        ...
        return rows.map((row) => {
            ...
            schema.forEach((column) => {
                ...
                if (value === null || value === undefined) {
                    ...
                    transformedRow[columnName] = new Date(value)
                        .toISOString()
                        .split("T")[0];
                    ...
                    transformedRow[columnName] = new String(
                        uint32ArrayToNumber(value, column.type2.scale),
                    );
                } else {
        ...
        if (match) {
            ...
            if (!registeredJsonUrls.includes(url)) {
                ...
                try {
                    ...
                    await conn.insertJSONFromPath(url, {
                        schema: "main",
                        name: url,
                    });
    ...
    <div class="svp-code-block">
        ...
        {#await results then r}
            <div class="my-4-mx-2">
                <div
                    class="tabulator"
                    use:tableAction={{ data: r.rows, columns: r.columns }}
                />
            </div>
        {:catch error}
            <div class="my-4-mx-2">
...
<style lang="css">
    .tabulator {
        ...
        margin-left: 0.5rem;
        margin-right: 0.5rem;
        font-family: "Iosevka Term SS08 Web", Consolas, "Lucida Console",
            Courier, monospace;
        font-size: 0.8em;
    }
